...
import { getStudentsFromUser } from '../../service/userApi';
...
export default function StudentListTable() {
    ...
    useEffect(() => {
        fetchStudents(currentPage, tabKey);
    }, [currentPage, tabKey])
    ...
    const fetchStudents = async (page, key) => {
        setLoading(true);
        try {
            const response = await getStudentsFromUser(page, key);
    ...
    const items = [
        {
            key: 'today',
            label: <span className="text-sm font-semibold">Today</span>,
        },
        {
            key: 'n1',
            label: <span className="text-sm font-semibold">N + 1</span>,
        },
        {
            key: 'n2',
            label: <span className="text-sm font-semibold">N + 2</span>,
        },
        {
            key: 'n3',
            label: <span className="text-sm font-semibold">N + 3</span>,
        },
        {
            key: 'n4',
            label: <span className="text-sm font-semibold">N + 4</span>,
        },
        {
            key: 'n5',
            label: <span className="text-sm font-semibold">N + 5</span>,
        },
        {
            key: 'non_contactable',
            label: <span className="text-sm font-semibold">Non Contactable</span>,
        },
    ];
    ...
    return (
        <>
            <Tabs activeKey={tabKey} items={items} onChange={onChange} centered={true} />
            {loading ? (
                ...
                <div className="w-full">
                    ...
                    <div className="mt-2">
                        ...
                        >
                            <Pagination current={currentPage} onChange={onChangePagination} total={Number(totalPages) * 10} showSizeChanger={false} itemRender={itemRender} />
                        </div>
                    </div>
                </div>
            )}
